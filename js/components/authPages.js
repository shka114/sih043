...
function formatAuthErrorMessage(err) {
  ...
  if (code === 429 || code === "over_email_send_rate_limit" || msg.toLowerCase().includes("rate limit") || msg.toLowerCase().includes("security purposes")) {
    return "Email rate limit reached for this hour. Please wait a few minutes before trying again, or sign in if already registered.";
  }
  if (msg.toLowerCase().includes("user already registered") || msg.toLowerCase().includes("already registered") || msg.toLowerCase().includes("already exists")) {
    return "An account with this email already exists. Please sign in or use Forgot Password.";
  }
  if (msg.toLowerCase().includes("email not confirmed") || code === "email_not_confirmed") {
    return "Your email is not confirmed yet. Please check your inbox for the verification link.";
  }
  if (msg.toLowerCase().includes("invalid login credentials") || msg.toLowerCase().includes("invalid_grant")) {
    return "Invalid email or password. Please verify your credentials.";
  }
  if (msg.toLowerCase().includes("password should be at least") || msg.toLowerCase().includes("weak password")) {
...
async function handleLoginSubmit(e) {
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = `<svg class="w-4 h-4 animate-spin inline mr-1 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10" stroke-opacity="0.25"></circle><path d="M12 2a10 10 0 0 1 10 10"></path></svg><span>Signing in...</span>`;
  }
  ...
  try {
    ...
    try {
      ...
      // If demo user doesn't exist yet in Supabase Auth, auto-register demo user
      if (loginErr.message && (loginErr.message.includes("Invalid login credentials") || loginErr.message.includes("User not found"))) {
        if (email.endsWith("@samadhan.org") || email.endsWith("@bitmesra.ac.in") || email.endsWith("@ranchiuniv.edu") || email.endsWith("@tatasteel.com") || email.endsWith("@pradan.net") || email.endsWith("@samadhansetu.gov.in")) {
          try {
            await supabaseSignUp(currentRole.name, email, password, currentRole.label);
            authResult = await supabaseSignIn(email, password);
          } catch (autoRegErr) {
            throw loginErr;
          }
    ...
    if (authResult && authResult.user) {
      ...
      const profile = await fetchPublicUserProfile(authUser.id);
      
      currentUser = {
        id: authUser.id,
        name: profile?.Name || authUser.user_metadata?.full_name || currentRole.name,
        email: authUser.email,
        role: currentRole.id,
        roleLabel: profile?.Role || authUser.user_metadata?.role || currentRole.label,
        org: currentRole.org
      };
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = `<i data-lucide="log-in" class="w-4 h-4"></i><span>Sign In as ${currentRole.label}</span>`;
...
async function handleRegisterSubmit(e) {
  ...
  if (btn) {
    btn.disabled = true;
    btn.innerHTML = `<svg class="w-4 h-4 animate-spin inline mr-1 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10" stroke-opacity="0.25"></circle><path d="M12 2a10 10 0 0 1 10 10"></path></svg><span>Creating Account...</span>`;
  }

  try {
    const data = await supabaseSignUp(name, email, password, currentRole.label);
    ...
    if (data && data.user) {
      if (data.session) {
        ...
        localStorage.setItem('samadhan_user', JSON.stringify(currentUser));
        if (typeof showToast === 'function') showToast("Account created successfully! Welcome to Samadhan Setu.", "success");
        ...
        if (typeof showToast === 'function') {
          showToast("Registration successful! A verification link has been sent to your email. Please check your inbox and confirm your account.", "info");
        }
    ...
    if (btn) {
      btn.disabled = false;
      btn.innerHTML = `<i data-lucide="user-plus" class="w-4 h-4"></i><span>Complete Registration as ${currentRole.label}</span>`;
      if (typeof lucide !== 'undefined') lucide.createIcons();
    }
  }
}
  ...
  modalRoot.innerHTML = `
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 modal-backdrop" onclick="closeModal(event)">
      <div class="bg-white rounded-3xl max-w-sm w-full p-6 shadow-2xl border border-[#E6DED2] modal-content-animated relative" onclick="event.stopPropagation()">
        ...
        <p class="text-xs text-[#64748B] mb-4">Enter your registered email to receive a secure password reset link.</p>
        <form onsubmit="handleResetPasswordSubmit(event)" class="space-y-3">
          <input type="email" id="reset-email-input" required placeholder="name@domain.com" class="w-full px-3.5 py-2.5 bg-[#FAF8F5] border border-[#E5DFD7] rounded-xl text-xs text-[#1C2421]" />
          <button type="submit" class="btn-primary-setu w-full py-2.5 text-xs font-bold cursor-pointer">Send Reset Link</button>
        </form>
      </div>
    </div>
  `;
...
async function handleResetPasswordSubmit(e) {
  ...
  try {
    ...
    closeModal();
    if (typeof showToast === 'function') showToast("Password reset link sent to your registered email.", "success");
  } catch (err) {
    console.error("Password reset error:", err);
    if (typeof showToast === 'function') showToast(err.message || "Failed to send reset link.", "error");
  }
}
